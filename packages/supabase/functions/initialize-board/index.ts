...
Deno.serve(async (req: Request) => {
  ...
  const {data: usersInLobby, error: usersInLobbySelectError} = await supabaseServiceClient.from('user_in_lobby').select('user_id');
  ...
  const {data: boardData, error: boardInsertError} = await supabaseServiceClient.from('board').insert({ turn_user_id: randomUserInLobby.user_id }).select();
  ...
  for (const userInLobby of usersInLobby) {
    const {data: cardTowerData, error: cardTowerInsertError} = await supabaseServiceClient.from('card_tower').insert({board_id: newBoard.id, user_id: userInLobby.user_id}).select();
    ...
    for (const cardToMoveFromBoardDeck of cardsToMoveFromBoardDeck) {
      const { error: cardInCardTowerInsertError} = await supabaseServiceClient.from('card_in_tower').insert({card_tower_id: newCardTower.id, card_number: cardToMoveFromBoardDeck.number}).select();
  ...
  for (const cardInBoardDeck of cardsInBoardDeck) {
    const {error: cardInBoardDeckInsertError} = await supabaseServiceClient.from('card_in_board_deck').insert({board_id: newBoard.id, card_number: cardInBoardDeck.number}).select();
